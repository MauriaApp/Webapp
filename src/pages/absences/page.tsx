...
import { getCurrentYearAbsences, getJustifiedAbsencesDuration, getTotalAbsencesDuration, getUnjustifiedAbsencesDuration } from "@/utils/absences";
import { AbsenceData, fetchAbsences, getAbsences } from "@/utils/api";
          ...
          {list.length === 0 ? (
            ...
            list.map((a, i) => {
              ...
              return (
                <AbsenceCard
                  key={key}
                  duration={(a as any).duration ?? (a as any).length ?? ""}
                  type={(a as any).type ?? ((a as any).justified ? "Absence excusée" : "Absence non excusée")}
                  course={(a as any).course ?? (a as any).subject ?? (a as any).module ?? ""}
                  time={(a as any).time ?? (((a as any).start && (a as any).end) ? `${(a as any).start} - ${(a as any).end}` : "")}
                  date={(a as any).date ?? (a as any).day ?? ""}
                  excused={!!((a as any).excused ?? (a as any).justified)}
                />
              )
            })
          )}
  ...
  return (
    <Card className="bg-white dark:bg-mauria-dark-card border-none shadow-md p-4">
      <div className="flex">
        ...
        <div className="flex-1">
          <div className={`text-lg font-medium ${excused ? "text-mauria-light-purple" : "text-mauria-light-accent"} dark:text-white`}>
            {type}
          </div>
